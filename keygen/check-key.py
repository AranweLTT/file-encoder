...
if __name__ == "__main__":
    ...
    private_inv = np.round(np.linalg.inv(private_matrix)*det*alpha).astype(int) % 256
    ...
    print_matrix((np.matmul(private_matrix,public_matrix)).astype(int) % 256, "private x public")
    print_matrix((np.matmul(public_matrix,private_matrix)).astype(int) % 256, "public x private")
    print_matrix((np.matmul(private_matrix,private_inv)).astype(int) % 256, "private x private_inv")
    print_matrix((np.matmul(public_matrix,public_inv)).astype(int) % 256, "public x public_inv")
